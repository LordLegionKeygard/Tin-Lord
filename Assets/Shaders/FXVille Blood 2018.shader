Shader "Particles/FXVille Blood 2018_Ortho"
{
    Properties
    {
        ...
        SubShader
        {
            ...
            Pass 
            {
                ...
                v2f vert (appdata v)
                {
                    ...
                    // Корректное добавление fallPos до преобразования в клип-пространство
                    float4 modifiedPos = v.vertex + fallPos;
                    float3 worldPos = mul(unity_ObjectToWorld, modifiedPos).xyz;
                    ...
                    #ifdef SPECULAR_REFLECTION_ON
                        // Получаем все необходимые векторы и матрицы для обработки отражённых нормалей //
                        float3 binormal = cross(v.normal, v.tangent.xyz) * v.tangent.w;								
                        float3x3 rotation = float3x3(v.tangent.xyz, binormal, v.normal);							
                        o.tangentToWorld = mul((float3x3)unity_ObjectToWorld, transpose(rotation));							

                        // Устанавливаем фиксированное направление обзора для ортографической камеры
                        float3 cameraForward = float3(0, 0, 1); // Предполагается, что камера смотрит вдоль положительной оси Z
                        o.viewDir = cameraForward;																	
                    #endif
                ...
                {	
                    ...
                    baseLighting = i.vertLight.xyz;
                        ...
                        float3 viewDir = normalize(i.viewDir); // Уже фиксированное направление

                        ////// Calculate Reflection UVs ///////
                        float3 reflectionVector = reflect(-i.viewDir, combinedNormals);

                        // Используем только x и y компоненты для UV, так как z фиксирована
                        float2 reflectionUVs = (reflectionVector.xy + 1.0) * 0.5; // Преобразуем из диапазона [-1,1] в [0,1]
                        reflectionUVs *= _ReflectionTex_ST.xy;
                        reflectionUVs += _ReflectionTex_ST.zw + _Time * 0.1; // Добавляем анимацию по времени, масштабируйте по необходимости
                    ...
                    col.xyz *= lerp(min(col.xyz * col.xyz * col.xyz * 0.3, 1.0), 0.71, edge);  /// Make sure this doesn't end up way over one
